...
Panel {
  ...
  readonly property var subscriptionState: provider ? (provider.subscription || null) : null
  ...
  function formatResetDateTime(w) {
    if (!w || w.resetAt === "") return ""
    var reset = new Date(w.resetAt)
    if (!isFinite(reset.getTime())) return ""

    var weekdays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
    var hour24 = reset.getHours()
    var hour12 = hour24 % 12 || 12
    var minute = String(reset.getMinutes()).padStart(2, "0")
    var date = weekdays[reset.getDay()] + " " + (reset.getMonth() + 1) + "/" + reset.getDate()
    if (reset.getFullYear() !== new Date(root.nowMs).getFullYear()) date += "/" + reset.getFullYear()
    return date + ", " + hour12 + ":" + minute + " " + (hour24 < 12 ? "AM" : "PM")
  }
  ...
  function subscriptionDate(raw) {
    var text = String(raw || "")
    if (text === "") return ""
    var exactDay = text.match(/^(\d{4})-(\d{2})-(\d{2})$/)
    if (exactDay) {
      var dayMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
      return dayMonths[Number(exactDay[2]) - 1] + " " + Number(exactDay[3]) + ", " + exactDay[1]
    }
  ...
  function subscriptionStatusLabel(s) {
    if (!s || s.status === "unknown") return "Unavailable"
    if (s.cancelAtPeriodEnd) return "Canceled"
    return s.status === "active" ? "Active" : "Inactive"
  }

  function subscriptionDetail(s) {
    if (!s || s.status === "unknown") return "Subscription status unavailable"
    if (s.status !== "active") return "No active subscription"
    if (s.nonRenewing) return "Prepaid account · does not renew"
    if (s.cancelAtPeriodEnd) {
      var endDate = subscriptionDate(s.endsAt)
      return endDate === "" ? "Canceled · access ends after this billing period" : "Canceled · access ends " + endDate
    }
    var renewal = subscriptionDate(s.renewsAt)
    if (renewal === "") return "Renewal date unavailable"
    return "Renews " + renewal
  }
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // ---------- Subscription ----------
          PanelSeparator {
            visible: subscriptionSection.visible
            foreground: root.foreground
          }

          Column {
            id: subscriptionSection
            visible: !!root.provider && !!root.subscriptionState
            width: parent.width
            ...
            Item {
              width: parent.width
              implicitHeight: Math.max(subscriptionLabel.implicitHeight, subscriptionValue.implicitHeight)
              ...
              Text {
                ...
                text: root.subscriptionStatusLabel(root.subscriptionState)
                color: root.subscriptionState && (root.subscriptionState.status === "inactive"
                  || root.subscriptionState.cancelAtPeriodEnd) ? root.urgent
                  : root.subscriptionState && root.subscriptionState.status === "active" ? root.foreground : root.dim
                font.family: root.fontFamily
            ...
            Text {
              width: parent.width
              text: root.subscriptionDetail(root.subscriptionState)
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              wrapMode: Text.WordWrap
  ...
  // A limit window: label and percentage, meter, and local reset time.
