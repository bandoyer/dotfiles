...
Item {
  ...
  function updateCommand(kind, agentIds) {
    var command = ["bash", root.localUpdater]
  ...
  function providerHasData(p) {
    return numberValue(p.totalPrompts) > 0 || numberValue(p.totalSessions) > 0
      ...
      || !!p.balance || (!!p.subscription && p.subscription.status !== "unknown")
  }
  ...
  function subscriptionValue(raw, record) {
    var value = raw && typeof raw === "object" ? raw : null
    var status = value ? String(value.status || "unknown").toLowerCase() : "unknown"
    if (status !== "active" && status !== "inactive") status = "unknown"

    // Records written before the subscription field existed still get a
    // sensible first paint while the new collectors finish their first run.
    if (!value) {
      if (String(record.tierLabel || "") !== "") status = "active"
    }

    return {
      status: status,
      renewsAt: value ? String(value.renewsAt || "") : "",
      endsAt: value ? String(value.endsAt || "") : "",
      cancelAtPeriodEnd: value ? value.cancelAtPeriodEnd === true : false,
      estimated: value ? value.estimated === true : false,
      nonRenewing: value ? value.nonRenewing === true : false,
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      subscription: subscriptionValue(record.subscription, record),
  ...
  // Device-scoped stats add up across machines. Account-scoped stats are
  // replicas of the same upstream truth on every synced device, so the widest
  // value wins — summing them would double every token per machine.
